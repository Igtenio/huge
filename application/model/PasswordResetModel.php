...
class PasswordResetModel
{
     ...
     *
     * @param $user_username_or_email string Username or user's email
     * @param $captcha string Captcha string
     *
     * @return bool success status
     */
    public static function requestPasswordReset($user_username_or_email, $captcha)
    {
        ...
        if (empty($user_username_or_email)) {
            Session::add('feedback_negative', Text::get('FEEDBACK_USERNAME_EMAIL_FIELD_EMPTY'));
            return false;
        }

        // check if that username exists
        $result = UserModel::getUserDataByUserNameOrEmail($user_username_or_email);
        if (!$result) {
        ...
        $token_set = self::setPasswordResetDatabaseToken($result->user_username, $user_password_reset_hash, $temporary_timestamp);
        if (!$token_set) {
            return false;
        }

        // ... and send a mail to the user, containing a link with username and token hash string
        $mail_sent = self::sendPasswordResetMail($result->user_username, $user_password_reset_hash, $result->user_email);
        if ($mail_sent) {
            return true;
        }
     ...
     * @return bool success status
     */
    public static function setPasswordResetDatabaseToken($user_username, $user_password_reset_hash, $temporary_timestamp)
    {
        ...
        $sql = "UPDATE users
                ...
                WHERE user_username = :user_username AND user_provider_type = :provider_type LIMIT 1";
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_password_reset_hash' => $user_password_reset_hash, ':user_username' => $user_username,
     ...
     * @return bool success status
     */
    public static function sendPasswordResetMail($user_username, $user_password_reset_hash, $user_email)
    {
        // create email body
        $body = Config::get('EMAIL_PASSWORD_RESET_CONTENT') . ' ' . Config::get('URL') .
                Config::get('EMAIL_PASSWORD_RESET_URL') . '/' . urlencode($user_username) . '/' . urlencode($user_password_reset_hash);
     ...
     * @return bool Success status
     */
    public static function verifyPasswordReset($user_username, $verification_code)
    {
        $database = DatabaseFactory::getFactory()->getConnection();
        ...
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_password_reset_hash' => $verification_code, ':user_username' => $user_username,
     ...
     * @return bool
     */
    public static function saveNewUserPassword($user_username, $user_password_hash, $user_password_reset_hash)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "UPDATE users SET user_password_hash = :user_password_hash, user_password_reset_hash = NULL,
                       user_password_reset_timestamp = NULL
                 WHERE user_username = :user_username AND user_password_reset_hash = :user_password_reset_hash
                       AND user_provider_type = :user_provider_type LIMIT 1";
        $query = $database->prepare($sql);
        $query->execute(array(
            ':user_password_hash' => $user_password_hash, ':user_username' => $user_username,
     ...
     */
    public static function setNewPassword($user_username, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        // validate the password
        if (!self::validateResetPassword($user_username, $user_password_reset_hash, $user_password_new, $user_password_repeat)) {
            return false;
        }
        ...
        if (self::saveNewUserPassword($user_username, $user_password_hash, $user_password_reset_hash)) {
     ...
     * @return bool
     */
    public static function validateResetPassword($user_username, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
     ...
     * @return bool
     */
    public static function saveChangedPassword($user_username, $user_password_hash)
    {
        $database = DatabaseFactory::getFactory()->getConnection();
     ...
     * @return bool
     */
    public static function changePassword($user_username, $user_password_current, $user_password_new, $user_password_repeat)
    {
        // validate the passwords
        if (!self::validatePasswordChange($user_username, $user_password_current, $user_password_new, $user_password_repeat)) {
            return false;
        }
        ...
        if (self::saveChangedPassword($user_username, $user_password_hash)) {
            Session::add('feedback_positive', Text::get('FEEDBACK_PASSWORD_CHANGE_SUCCESSFUL'));
            return true;
        } else {
            Session::add('feedback_negative', Text::get('FEEDBACK_PASSWORD_CHANGE_FAILED'));
     ...
     * @return bool
     */
    public static function validatePasswordChange($user_username, $user_password_current, $user_password_new, $user_password_repeat)
    {
        $database = DatabaseFactory::getFactory()->getConnection();

        $sql = "SELECT user_password_hash, user_failed_logins FROM users WHERE user_username = :user_username LIMIT 1;";
        $query = $database->prepare($sql);
        $query->execute(array(
